...
const uuidv4 = typeof crypto !== 'undefined' && (crypto as any).randomUUID
  ? () => (crypto as any).randomUUID()
  : uuidv4_import

type CallType = 'video' | 'audio'
type CallStatus = 'idle' | 'calling' | 'ringing' | 'accepted' | 'in-call' | 'declined' | 'ended' | 'cancelled'
...
async function getToken(callId: string, roomId: string) {
  const { data, error } = await supabase.functions.invoke('generate-token', { body: { callId, roomId } })
...
export const useCall = create<CallState>((set, get) => ({
  ...
  async startCall(receiverId, callType) {
    ...
    if (!callerId) { set({ status: 'idle', error: 'Sem sessão' }); alert('Faça login para ligar'); return }

    const roomId = 'room_' + uuidv4()
    const { data: call, error } = await supabase.from('video_calls')
      .insert({ caller_id: callerId, receiver_id: receiverId, room_id: roomId, call_type: callType, status: 'calling' })
      .select('*').single()
    if (error || !call) { console.error('[startCall]', error); set({ status: 'idle', error: error?.message }); alert('Erro ao iniciar: ' + (error?.message ?? '')); return }
    ...
    const ch = supabase.channel('call-status-' + call.id)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'video_calls', filter: `id=eq.${call.id}` },
        async (payload) => {
          ...
          if (row.status === 'accepted') {
            ...
            } catch (e: any) { console.error(e); set({ error: e.message }); alert('Falha ao conectar: ' + e.message) }
          } else if (row.status === 'declined' || row.status === 'ended' || row.status === 'cancelled') {
  ...
  async acceptCall(callId, container) {
    try {
      const { data: vc, error } = await supabase.from('video_calls')
        .update({ status: 'accepted', started_at: new Date().toISOString() }).eq('id', callId).select('*').single()
      if (error || !vc) { console.error('[acceptCall]', error); set({ error: error?.message }); alert('Erro ao aceitar: ' + (error?.message ?? 'RLS?')); return }
      const token = await getToken(callId, vc.room_id)
      if (container) await connectAndAttach(token, container)
      set({ status: 'in-call', callId, currentRoomId: vc.room_id, incoming: null })
    } catch (e: any) { console.error('[acceptCall]', e); set({ error: e.message }); alert('Falha ao atender: ' + e.message + '\nA função generate-token está deployada?') }
  },
  ...
  async listenInvites(userId) {
    ...
    const ch = supabase.channel('calls-invites-' + userId)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'video_calls' },
        (payload) => {
          const row: any = payload.new
          if (row.receiver_id === userId && row.status === 'calling') {
            set({ incoming: { call_id: row.id, caller_id: row.caller_id, call_type: row.call_type }, status: 'ringing' })
            const audio = new Audio('/sounds/ringtone.mp3'); audio.loop = true; audio.play().catch(() => {})
          }
        })
      .subscribe()
