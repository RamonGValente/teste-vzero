...
export default function Messages() {
  ...
  // Auto-seleciona a primeira conversa quando carregar
  useEffect(() => {
          ...
          <div className="flex-1 overflow-auto divide-y">
            {(filteredConvs || []).map((c) => {
              ...
              return (
                <button
                  key={c.id}
                  className={cn("w-full p-3 flex items-center gap-3 text-left hover:bg-muted/60", selectedConversation === c.id && "bg-muted")}
