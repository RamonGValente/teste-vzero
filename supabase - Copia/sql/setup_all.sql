...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='attention_calls') THEN
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'delete_expired_messages') THEN
    PERFORM cron.schedule('delete_expired_messages', '*/1 * * * *', $$DELETE FROM public.messages WHERE auto_delete_at IS NOT NULL AND auto_delete_at <= now();$$);
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name='message_files_message_id_fkey' AND table_schema='public' AND table_name='message_files') THEN
